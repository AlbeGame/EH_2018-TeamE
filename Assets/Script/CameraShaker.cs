...
public class CameraShaker : MonoBehaviour
{
    ...
    IEnumerator Shake()
    {
        while (true)
        {
            if (direction)
            {
                transform.rotation = Quaternion.RotateTowards(originalRotation, originalRotation *Quaternion.AngleAxis(ShakeForce, Vector3.forward), Time.deltaTime* ShakeForce);
                freqConuter += Time.deltaTime;
                if (freqConuter > ShakeFrequence)
                    direction = !direction;
            }
            else
            {
